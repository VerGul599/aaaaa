...
void countPosNegZero(int ar[], int siz) 
{
    ...
    cout << "Положительные: " << pos << ", Отрицательные: " << neg << ", Нулевые: " << zero;
}
...
void playGame(int secretNumber, int attempts)
{
    ...
    if (bulls == 4) 
    {
        std::cout << "Поздравляем, вы угадали число! Количество попыток: " << attempts ;
    }
    else 
    {
...
int main()
{
    ...
    cout << "Разница в днях между датами: " << difference;
    ...
    cout << "Среднее арифметическое элементов массива: " << avg;
    ...
    cout << "Максимальный элемент на главной диагонали: " << maxDiagonalElement(matrix, size);
    cout << "Минимальный элемент на главной диагонали: " << minDiagonalElement(matrix, size);

    cout << "задание 2";
    int num1 = 48;
    int num2 = 18;

    cout << "Наибольший общий делитель чисел " << num1 << " и " << num2 << " равен: " << gcd(num1, num2);
